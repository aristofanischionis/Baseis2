...
void Insert_First(int fileDesc, void *value1, void *value2){
    ...
    //BF_BLOCK_SIZE -7 because 4 bytes for extra pointer + 3 bytes for ids (e2\0)
    numberofkeys = (BF_BLOCK_SIZE - 7) / (open_arr[pos].attrLength1 + 4);

    CALL_OR_DIE(BF_AllocateBlock(fileDesc, block1)); //root block
    data1 = BF_Block_GetData(block1);
    data1[0] = 'e';
    data1[1] = '2'; //to root = 1, to 2 einai to prwto mplok dedomenwn, ara deixnoume sto prwto mplok dedomenwn
    data1[2] = '\0';
    data1 += 2;
    memcpy(data1, value1, open_arr[pos].attrLength1);
    
    data1+=sizeof(value1); //giati?
